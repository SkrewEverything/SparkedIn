...
import ReceivedDataGrid from "./ReceivedApplications";
...
const renderActions = (params) => (
  <Stack direction="row" spacing={1}>
    <Button color="error" disabled={params.row.status !== "Applied"} onClick={() => updateApplicationStatus(JSON.parse(localStorage.getItem("selectedUser")).userId, params.row.jobId, "Withdraw")}>Withdraw</Button>
  </Stack>
);
...
const columns = [
  ...
  },
  {
    ...
    width: 100,
    renderCell: (params) => (
      <Link href={"/jobs/" + params.row.jobId}>{params.row.jobId}</Link>
    )
  },
  {
    field: 'experience',
    headerName: 'Experience Needed (Years)',
    type: 'number',
    width: 150,
    align: 'center',
  },
...
export default function DataGridDemo() {
  const [appliedToList, setAppliedToList] = React.useState([]);

  React.useEffect(() => {
    async function getList() {
      const appliedTo = await getAppliedToJobs();
      for (let i = 0; i < appliedTo.length; i++) {
        appliedTo[i].id = i + 1;
      }
      console.log("DATA: ", appliedTo)
      setAppliedToList(appliedTo);
    }

    getList();
  }, []);
  
  console.log("APPLIED: ", appliedToList);
  return (
    <>
    <Box sx={{ 
        height: 'auto', 
        ...
        p: 2, 
        borderRadius: 2 }}>
          <Typography variant='h1' color="GrayText">Jobs you have applied</Typography>
          <br />
      <DataGrid
        rows={appliedToList}
        columns={columns}
...
async function  getAppliedToJobs () {
  const selectedUser = localStorage.getItem("selectedUser");
  const response = await fetch('http://127.0.0.1:5000/api/applications/' + JSON.parse(selectedUser).userId)
  if (!response.ok) {
